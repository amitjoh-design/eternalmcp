...
export function CallLogs({ installedMcpId }: CallLogsProps) {
  ...
  const toolIcon = (name: string) =>
    name === 'send_email' ? <Mail size={11} className="shrink-0" /> : <FileText size={11} className="shrink-0" />

  const toolLabel = (name: string) =>
    name === 'send_email' ? 'Send Email' : name === 'create_draft' ? 'Create Draft' : name

  // Simplify user-agent to human-readable string
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="bg-surface-2 rounded-lg px-3 py-2.5 text-xs"
                >
                  {/* Row 1: status + tool + time */}
                  <div className="flex items-center gap-2">
                    {log.status === 'success'
                      ? <CheckCircle2 size={11} className="text-green-400 shrink-0" />
                      : <XCircle size={11} className="text-red-400 shrink-0" />}
                    <span className="flex items-center gap-1 text-text-primary font-medium">
                      {toolIcon(log.tool_name)}
                      {toolLabel(log.tool_name)}
                    </span>
                    <span className="ml-auto text-muted" title={format(new Date(log.created_at), 'PPpp')}>
                      {formatDistanceToNow(new Date(log.created_at), { addSuffix: true })}
                    </span>
                  </div>

                  {/* Row 2: to + subject */}
                  {(log.to_address || log.subject) && (
                    <div className="mt-1 text-muted truncate">
                      {log.to_address && <span className="text-text-secondary">→ {log.to_address}</span>}
                      {log.subject && <span className="ml-2 italic">{log.subject}</span>}
                    </div>
                  )}
              ...
              <p className="text-[10px] text-muted text-center pt-1">Showing last 25 calls</p>
            </div>
          )}
        </div>
      )}

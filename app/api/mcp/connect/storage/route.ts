// Install Storage Manager MCP (no OAuth needed)
// GET /api/mcp/connect/storage?userId=xxx
...
export async function GET(req: NextRequest) {
  ...
  const { data: existing } = await db
    ...
    .select('mcp_token')
    .eq('user_id', userId)
    .eq('mcp_slug', 'storage-manager')
    .single()

  if (existing) {
    return NextResponse.redirect(`${appUrl}/dashboard?tab=mcps&mcp_connected=storage-manager`)
  }
